...
c1	set	-1.0/@cos(6.28319*frq)*(1.0-@sin(6.28319*frq))
	...
	move		    #fse,r5		    ; Fractional Spaced Equalizer data pointer
	move		    #fseN-1,m5
...
; make decision and calculate decision error
	neg	a	    #-refLev,x1 	    ; x1 = alpha*(desired-filtered)
	jpl	<in1
	move		    #refLev,x1
in1	add	x1,a	    #alpha,x0
	move		    x1,y:<d1
	move	a,x1
	mpyr	x0,x1,a     #fsetaps,r0
	move	a,x1
	move	r0,r1

; fractional Spaced Equalizer (FSE) - Adaptation part (LMS)
	move		    x:(r0)+,a	y:(r5)+,y0  ; get x(0), c0
	do	#N,incupd
	macr	x0,x1,a     x:(r0)+,x0	y:(r5)+,y0  ; c = c + e(n) * x(n)
	tfr	x0,a	    a,x:(r1)+		    ; copy c, save new c
incupd
	move		    (r5)-		    ; readjust sample pointer
	move		    (r5)-
